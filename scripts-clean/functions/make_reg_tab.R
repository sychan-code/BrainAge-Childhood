make_reg_tab <- function(inputdf,pred) {
  tab <- inputdf %>%
    ...
    select(Type, Feature, "β estimate [95% CI]", std.error, statistic, p.value, stars) %>%
    arrange(Type, desc(Feature)) %>%
    flextable() %>%
    colformat_double(
    ...
    merge_v(j = "Type") %>%             # Merges the Sex column vertically
    valign(j = "Type", v = "top") %>%   # Puts the label at the top of the merge  
